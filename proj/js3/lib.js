...
if (!(this instanceof c || this.constructor == c)) { return new a()};
this.constructor=c;
//alert(f.apply)
...
this.constructor = c;
if(r != this) for(var i in r) this[i] = r[i];
}
this.Super = Super;
return f.apply(this, arguments) || this;
}
for(i in _super) {c[i] = _super[i]};
for(i in _static) {c[i] = _static[i]};
...
neg: function() {var v = this.copy(); v.mulSelf(-1); return v}
}, function(a){
var L;
//alert(a)
if(!a) a = [];
...
Transformator: withProto(lib, {toString: function(){return 'Transformator:{\n' + this.m + '\n' + this.v +'\n}'}, mulMx: function(m){this.m.mulSelf(m); this.v.mulSelf(m); return this}, rot: function(a, d){with(Vectors.SqMatrix){var m = rotMx(a, d); return this.mulMx(m)}}, mov: function(v){this.v.addSelf(v); return this}, rotAround: function(v, a, d) {}, proceedSelf: function(a){var i; for(i=0; i<a.length; i++) {if(a[i].mulSelf) a[i].mulSelf(this.m); if(a[i].addSelf) a[i].addSelf(this.v)}; return a}, proceed: function(a){var i, b = [], c; for(i=0; i<a.length; i++) {if(!a[i]) continue; c = a[i].constructor; if(c.mul) {b[i] = c.mul(a[i], this.m); if(b[i].addSelf) b[i].addSelf(this.v)}}}}, function(){
...
Graphics = withProto(lib, {bindView: function(C){C = getView(C); if(C == '[NONE]') C = this.constructor.canvView(); this.canv = C; return C}, getIndex: function(x, y){

return this._iData.width*y+x;
}}, function(d, s){d = d || [0, 1];
 if(d.split){d = d.split(''); d = d.map(function(c){switch(c){case 'x':return 0; case 'y':return 1; case 'z':return 2; default:return 3;}})};
 var w = d[0], h = d[1], i;
 var sq = h*w;
 
 this._iData = {height: h, width: w, data: new Array(sq*4)};
 var pixels = new Array(sq);
 this.pixels = pixels;
 for(i = 0; i < sq; i++) pixels[i] = new this.constructor.Color(this._iData, i*4);
 
 
 }, {Color: withProto(lib, {getValues: function(){var S = this.srcs; return S[0].slice(S[1], S[1] + 4 - this.noAlpha)}, unbind: function() {this.srcs = [this.getValues, 0]}, copy: function(){with (this) {return new constructor(getValues(), 0, noAlpha, isHex)}}, toString: function(){if(!this.isHex){var v = this.getValues(); if(v[3])v[3]/=255; return "rgb"+(this.noAlpha?'':'a')+'('+v.join(", ")+')'}}, getAlpha: function(floatVal) {return (this.noAlpha*255 || this.srcs[0][this.srcs[1]+3])/(floatVal?255:1)}, paintSelf: function(c){var ch, aa = (1 - c.getAlpha(1)) * this.getAlpha(1), S1 = this.srcs, S2 = c.srcs; for(ch = 0; ch < 3; ch++){S1[0][S1[1] + ch] = S2[0][S2[1] + ch] + aa * (S1[0][S1[1] + ch] - S2[0][S2[1] + ch])}; if(!this.noAlpha) S1[0][S1[1]+3] = (aa + c.getAlpha(1)) * 255}, painted: function(c) {var r = this.copy(); r.paintSelf(c); return r}}, function(arr, off, noAlpha, isHex){off = off || 0; this.noAlpha = !!noAlpha; this.isHex = !!isHex; this.srcs = [arr, off]}, {}), canvView: subClass(View, {}, function(){var C = document.createElement("canvas"); this.Super(C); this.CTX = C.getContext("2d")}, {})});
